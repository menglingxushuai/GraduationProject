...
- (void)showInsertImageDialogFromDeviceWithScale:(CGFloat)scale alt:(NSString *)alt {
    ...
    oss.imgBlock = ^(NSString *imgUrl) {
        DLog(@"%@", imgUrl);
        
        
         [self showInsertImageDialogWithLink:imgUrl alt:nil];
        
    };
    [self presentViewController:oss animated:YES completion:nil];
}
...
- (void)keyboardWillShowOrHide:(NSNotification *)notification {
    ...
    self.editorView.scrollView.showsVerticalScrollIndicator = NO;
    // Orientation
    ...
    if ([notification.name isEqualToString:UIKeyboardWillShowNotification]) {
        ...
        [UIView animateWithDuration:duration delay:0 options:animationOptions animations:^{
            ...
            self.editorViewFrame = CGRectMake(self.editorView.x, self.editorView.y, self.editorView.width, self.editorView.height - keyboardHeight - sizeOfToolbar - extraHeight);
